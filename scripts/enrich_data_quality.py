...
quality = {
    ...
    'restTravel': {'status': status(len(rest) >= len(matches), bool(rest)), 'coveredGroupMatches': len(rest), 'source': 'embedded schedule and venue coordinates'},
...
if data.get('dataQuality') != quality:
    data['dataQuality'] = quality
    data['generatedAt'] = quality['updatedAt']
    ...
    changed = False
print(json.dumps({'changed': changed, 'dataQuality': {k:v.get('status') for k,v in quality.items() if isinstance(v, dict) and 'status' in v}}, indent=2))
